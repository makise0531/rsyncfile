...
const int ndoppler = 1200;//2019/11/20
Double_t all_doppler  = 12.;//2019/11/20
Double_t divide_doppler = all_doppler / ndoppler;
Double_t TK = 300;
//------------------------------------

const Int_t Npx = 12000;
Double_t range[2] = {0.1, 12.}; //2019/11/20
...
void sp_paratext(){
        ...
        //Char_t s_file[100] = "./resopara_swave_nega.txt";
        //Char_t s_file[100] = "./resopara_swave_275.txt";
        ...
        //Char_t p_file[100] = "./resopara_pwave_117Sn.txt";
...
void reset_func(){
        Es[npara] = {0}; // resonance energy
        Ggs[npara]       = {0};// resonance width (exit channel)
        Gns2g[npara]     = {0};// resonacne width (entrance channel)
        Js[npara]        = {0};// total spin of compound state
        ns = 0.; // number of s-wave resonance

        Ep[npara] = {0}; // resonance energy
        Ggp[npara]       = {0};// resonance width (exit channel)
        Gnp2g[npara]     = {0};// resonacne width (entrance channel)
        Jp[npara]        = {0};// total spin of compound state
        np = 0; // number of p-wave resonance
        return;
}
...
complex<Double_t> V1s275cal(Double_t E, Double_t Es, Double_t Gns2g,  Double_t Ggs, Double_t Js){
        // Gns2g: 2gGamma_n(s-wave) resonance width of neutron (Entrance channel)
        // Ggs: Gamma_gamma(s-wave) resonance width of gamma-ray (Exit channel)
        // Js: total spin of compound state of s-wave resonance
        complex<Double_t> V1s275;
        complex<Double_t> V1s275_total;
        ...
        for(int i = 0; i < ndoppler; i++) {
                ...
                V1s275 = complex<Double_t> (Ex - Es, (0.5*Gns2g/gs + Ggs)/2.);
                V1s275 = -1.0e14 * sqrt(pi/ks/k) * sqrt(0.5*Gns2g*Ggs) / V1s275;

                V1s275_total += exp(-pow((Ex - E)/delta, 2.)) * sqrt(Ex/E) * V1s275;
        }

        return coefficient * V1s275_total * divide_doppler;
}

complex<Double_t> V1negacal(Double_t E, Double_t Es, Double_t Gns2g,  Double_t Ggs, Double_t Js){
        // Gns2g: 2gGamma_n(s-wave) resonance width of neutron (Entrance channel)
        // Ggs: Gamma_gamma(s-wave) resonance width of gamma-ray (Exit channel)
        // Js: total spin of compound state of s-wave resonance
        complex<Double_t> V1nega;
        complex<Double_t> V1nega_total;
        ...
        for(int i = 0; i < ndoppler; i++) {
                ...
                V1nega = complex<Double_t> (Ex - Es, (0.5*Gns2g/gs + Ggs)/2.);
                V1nega = -1.0e14 * sqrt(pi/ks/k) * sqrt(0.5*Gns2g*Ggs) / V1nega;

                V1nega_total += exp(-pow((Ex - E)/delta, 2.)) * sqrt(Ex/E) * V1nega;
        }

        return coefficient * V1nega_total * divide_doppler;
}

complex<Double_t> V1p453cal(Double_t E, Double_t Es, Double_t Gns2g,  Double_t Ggs, Double_t Js){
        // Gns2g: 2gGamma_n(s-wave) resonance width of neutron (Entrance channel)
        // Ggs: Gamma_gamma(s-wave) resonance width of gamma-ray (Exit channel)
        // Js: total spin of compound state of s-wave resonance
        complex<Double_t> V1p453;
        complex<Double_t> V1p453_total;
        ...
        for(int i = 0; i < ndoppler; i++) {
                ...
                V1p453 = complex<Double_t> (Ex - Es, (0.5*Gns2g/gs + Ggs)/2.);
                V1p453 = -1.0e14 * sqrt(pi/ks/k) * sqrt(0.5*Gns2g*Ggs) / V1p453;

                V1p453_total += exp(-pow((Ex - E)/delta, 2.)) * sqrt(Ex/E) * V1p453;
        }

        return coefficient * V1p453_total * divide_doppler;
}
...
complex<Double_t> V2s275cal(Double_t E, Double_t Ep, Double_t Gnp2g, Double_t Ggp, Double_t Jp, Double_t j, Double_t phi){
        complex<Double_t> V2s275;
        complex<Double_t> V2s275_total;
        ...
        for(int i = 0; i < ndoppler; i++) {
                ...
                V2s275 = complex<Double_t> (Ex - Ep, (0.5*Gnp2g/gp + Ggp)/2.);
                V2s275 = -1.0e14 * sqrt(pi/kp/k) * sqrt(0.5*Gnp2g*Ggp) / V2s275;

                if(j == 1./2.) {
                        V2s275 = V2s275 * cos(phi*pi/180.);
                }else if(j == 3./2.) {
                        V2s275 = V2s275 * sin(phi*pi/180.);
                }

                V2s275_total += exp(-pow((Ex - E)/delta, 2.)) * sqrt(Ex/E) * V2s275;
        }

        return coefficient * V2s275_total * divide_doppler;
}

complex<Double_t> V2negacal(Double_t E, Double_t Ep, Double_t Gnp2g, Double_t Ggp, Double_t Jp, Double_t j, Double_t phi){
        complex<Double_t> V2nega;
        complex<Double_t> V2nega_total;
        ...
        for(int i = 0; i < ndoppler; i++) {
                ...
                V2nega = complex<Double_t> (Ex - Ep, (0.5*Gnp2g/gp + Ggp)/2.);
                V2nega = -1.0e14 * sqrt(pi/kp/k) * sqrt(0.5*Gnp2g*Ggp) / V2nega;

                if(j == 1./2.) {
                        V2nega = V2nega * cos(phi*pi/180.);
                }else if(j == 3./2.) {
                        V2nega = V2nega * sin(phi*pi/180.);
                }

                V2nega_total += exp(-pow((Ex - E)/delta, 2.)) * sqrt(Ex/E) * V2nega;
        }

        return coefficient * V2nega_total * divide_doppler;
}

complex<Double_t> V2p453cal(Double_t E, Double_t Ep, Double_t Gnp2g, Double_t Ggp, Double_t Jp, Double_t j, Double_t phi){
        complex<Double_t> V2p453;
        complex<Double_t> V2p453_total;
        ...
        for(int i = 0; i < ndoppler; i++) {
                ...
                V2p453 = complex<Double_t> (Ex - Ep, (0.5*Gnp2g/gp + Ggp)/2.);
                V2p453 = -1.0e14 * sqrt(pi/kp/k) * sqrt(0.5*Gnp2g*Ggp) / V2p453;

                if(j == 1./2.) {
                        V2p453 = V2p453 * cos(phi*pi/180.);
                }else if(j == 3./2.) {
                        V2p453 = V2p453 * sin(phi*pi/180.);
                }

                V2p453_total += exp(-pow((Ex - E)/delta, 2.)) * sqrt(Ex/E) * V2p453;
        }

        return coefficient * V2p453_total * divide_doppler;
}
...
Double_t a0s275cal(Double_t E, Double_t Ene, Double_t gGn2, Double_t Gg, Double_t J, Double_t j, Double_t phi, Int_t Vpattern){
        // Vpattern = 0 (s-wave),  Vpattern =1 (p-wave)
        complex<Double_t> V1s275cal(Double_t, Double_t, Double_t, Double_t, Double_t);
        complex<Double_t> V2s275cal(Double_t, Double_t, Double_t, Double_t, Double_t, Double_t, Double_t);

        Double_t a0s275 = 0.;
        if(Vpattern == 0) {
                a0s275 = abs(V1s275cal(E, Ene, gGn2, Gg, J) * conj(V1s275cal(E, Ene, gGn2, Gg, J)));
        }else{
                a0s275 = abs(V2s275cal(E, Ene, gGn2, Gg, J, j, phi) * conj(V2s275cal(E, Ene, gGn2, Gg, J, j, phi)));
        }
        return a0s275;
}

Double_t a0s994cal(Double_t E, Double_t Ene, Double_t gGn2, Double_t Gg, Double_t J, Double_t j, Double_t phi, Int_t Vpattern){
        // Vpattern = 0 (s-wave),  Vpattern =1 (p-wave)
        complex<Double_t> V1cal(Double_t, Double_t, Double_t, Double_t, Double_t);
        complex<Double_t> V2cal(Double_t, Double_t, Double_t, Double_t, Double_t, Double_t, Double_t);
        ...
        Double_t a0s994 = 0.;
        if(Vpattern == 0) {
                a0s994 = abs(V1cal(E, Ene, gGn2, Gg, J) * conj(V1cal(E, Ene, gGn2, Gg, J)));
        }else{
                a0s994 = abs(V2cal(E, Ene, gGn2, Gg, J, j, phi) * conj(V2cal(E, Ene, gGn2, Gg, J, j, phi)));
        }
        return a0s994;
}

Double_t a0negacal(Double_t E, Double_t Ene, Double_t gGn2, Double_t Gg, Double_t J, Double_t j, Double_t phi, Int_t Vpattern){
        // Vpattern = 0 (s-wave),  Vpattern =1 (p-wave)
        complex<Double_t> V1negacal(Double_t, Double_t, Double_t, Double_t, Double_t);
        complex<Double_t> V2negacal(Double_t, Double_t, Double_t, Double_t, Double_t, Double_t, Double_t);

        Double_t a0nega = 0.;
        if(Vpattern == 0) {
                a0nega = abs(V1negacal(E, Ene, gGn2, Gg, J) * conj(V1negacal(E, Ene, gGn2, Gg, J)));
        }else{
                a0nega = abs(V2negacal(E, Ene, gGn2, Gg, J, j, phi) * conj(V2negacal(E, Ene, gGn2, Gg, J, j, phi)));
        }
        return a0nega;
}

Double_t a0p453cal(Double_t E, Double_t Ene, Double_t gGn2, Double_t Gg, Double_t J, Double_t j, Double_t phi, Int_t Vpattern){
        // Vpattern = 0 (s-wave),  Vpattern =1 (p-wave)
        complex<Double_t> V1p453cal(Double_t, Double_t, Double_t, Double_t, Double_t);
        complex<Double_t> V2p453cal(Double_t, Double_t, Double_t, Double_t, Double_t, Double_t, Double_t);

        Double_t a0p453 = 0.;
        if(Vpattern == 0) {
                a0p453 = abs(V1p453cal(E, Ene, gGn2, Gg, J) * conj(V1p453cal(E, Ene, gGn2, Gg, J)));
        }else{
                a0p453 = abs(V2p453cal(E, Ene, gGn2, Gg, J, j, phi) * conj(V2p453cal(E, Ene, gGn2, Gg, J, j, phi)));
        }
        return a0p453;
}
...
Double_t a0s275func(Double_t E, Double_t phi){
        Double_t a0s275 = 0.;
        Double_t j[2] = {1./2., 3./2.};
        //s波
        for(int i=1; i < 2; i++) {
                a0s275 += a0s275cal(E, Es[i], Gns2g[i], Ggs[i], Js[i], 0., 0., 0);
        }
        return a0s275;
}

Double_t a0s994func(Double_t E, Double_t phi){
        Double_t a0s994 = 0.;
        Double_t j[2] = {1./2., 3./2.};
        //s波
        for(int i=2; i < 3; i++) {
                a0s994 += a0s994cal(E, Es[i], Gns2g[i], Ggs[i], Js[i], 0., 0., 0);
        }
        return a0s994;
}

Double_t a0negafunc(Double_t E, Double_t phi){
        Double_t a0nega = 0.;
        Double_t j[2] = {1./2., 3./2.};
        //s波
        for(int i=0; i < 1; i++) {
                a0nega += a0negacal(E, Es[i], Gns2g[i], Ggs[i], Js[i], 0., 0., 0);
        }

        return a0nega;
}

Double_t a0p453func(Double_t E, Double_t phi){
        Double_t a0p453 = 0.;
        Double_t j[2] = {1./2., 3./2.};

        for(int i=0; i < 1; i++) {
                for(int n=0; n < 2; n++) {
                        a0p453 += a0p453cal(E, Ep[i], Gnp2g[i], Ggp[i], Jp[i], j[n], phi, 1);
                }
        }
        return a0p453;
}
...
void eachTermDraw(Double_t phi, int mode){
        ...
        if(mode==0) {
                ccc->cd();
                cout<<"Fight!!"<<endl;
                cout<<"| Drawing a0 all resonance ..."<<endl;
                ...
                a0->SetLineWidth(4);
                a0->SetLineStyle(1);
                //a0->GetXaxis()->SetRangeUser(0.1, 10.0);
                ...
                TF1 *a0s275 = new TF1("a0_s275", Form("a0s275func(x, %lf)", phi), DrawRange[0], DrawRange[1]);
                a0s275->SetNpx(Npx);
                cout<<"Fight!!"<<endl;
                cout<<"| Drawing a0 s-wave 27.5 eV ..."<<endl;
                a0s275->Draw("l");
                a0s275->SetLineColor(kBlue);
                a0s275->SetLineWidth(4);
                a0s275->SetLineStyle(1);
                ...
                a0s275->GetXaxis()->SetRangeUser(0.1, 10.0);
                a0s275->GetYaxis()->SetRangeUser(0.001, 5.0);
                a0s275->GetXaxis()->SetTitle("neutron Energy [eV]");
                a0s275->GetYaxis()->SetTitle("cross section [barn]");

        }else if(mode==2) {
                ccc->cd();
                TF1 *a0s994 = new TF1("a0_s994", Form("a0s994func(x, %lf)", phi), DrawRange[0], DrawRange[1]);
                a0s994->SetNpx(Npx);
                cout<<"Fight!!"<<endl;
                cout<<"| Drawing a0 s-wave 99.4 eV ..."<<endl;
                a0s994->Draw("lsame");
                a0s994->SetLineColor(kMagenta);
                a0s994->SetLineWidth(4);
                a0s994->SetLineStyle(1);
                //a0nega->GetXaxis()->SetRangeUser(0.1, 10.0);

        }else if(mode==3) {
                ccc->cd();
                TF1 *a0nega = new TF1("a0_nega", Form("a0negafunc(x, %lf)", phi), DrawRange[0], DrawRange[1]);
                a0nega->SetNpx(Npx);
                cout<<"Fight!!"<<endl;
                cout<<"| Drawing a0 nega ..."<<endl;
                a0nega->Draw("lsame");
                a0nega->SetLineColor(kGreen);
                a0nega->SetLineWidth(4);
                a0nega->SetLineStyle(1);
                //a0nega->GetXaxis()->SetRangeUser(0.1, 10.0);

        }else if(mode==4) {
                ccc->cd();
                TF1 *a0p453 = new TF1("a0_p453", Form("a0p453func(x, %lf)", phi), DrawRange[0], DrawRange[1]);
                a0p453->SetNpx(Npx);
                cout<<"Fight!!"<<endl;
                cout<<"| Drawing a0p453 term..."<<endl;
                a0p453->Draw("lsame");
                a0p453->SetLineColor(kRed);
                a0p453->SetLineWidth(4);
                a0p453->SetLineStyle(1);
                //a0p453->GetXaxis()->SetRangeUser(0.1, 10.0);
        }
...
void eachTermAsy(){
        ...
        a0->GetXaxis()->SetRangeUser(4.03, 5.03);
